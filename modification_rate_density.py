...
output_graph_path = "../../../../data2/tom/RNA/modification_rate_transcript_position_graph.png"
df_original = pd.read_csv(input_file_path, sep='\t', header=0)
df2 = df_original[df_original['GMM_logit_pvalue'].astype(str) != "nan"]
df_filtered = df2.loc[df2['ref_id'].str.contains("mrna")]
#df = df3.loc[(df3['GMM_logit_pvalue'].astype(float) <= 0.01)]
#df_filtered = df.loc[(df['relative_average_modification_rate'].astype(float) >= 20)]
#df_filtered = df
#transcript_lengths = {}
...
df_filtered = df_filtered.loc[(df_filtered['mod_rate_change'].astype(float) <= 500) & (df_filtered["mod_rate_change"].astype(float) >= -500)]
df_filtered = df_filtered.loc[(df_filtered["mod_rate_change"].astype(float) >= 150) | (df_filtered["mod_rate_change"].astype(float) <= - 150)]
#sns.lineplot
#sns.lmplot
plot = sns.lineplot(data = df_filtered, x = "relative_position_in_transcript", y = "mod_rate_change") #, fit_reg = False)
#fig = plt.gcf()
#fig.set_size_inches( 16, 10) #16, 10, 40, 
sns.set(font_scale = 1)
plot.set(xlabel = 'Relative position in transcript (%)', ylabel = "Change in modification rate")
#plot.set_xlim([0, 100])
#plot.set_size_inches( 6, 4)
plot.set(title = "Relative position in mRNA against modification rate change")
#plot = plot.get_figure()
#plot.legend.set_title("Transcript id")
plot.figure.savefig(output_graph_path)
