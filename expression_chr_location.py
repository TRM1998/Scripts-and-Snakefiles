...
output_file_path = "../../data2/tom/RNA/expression_chr.tsv"
output_graph_path = "../../data2/tom/RNA/expression_chr.png"
...
input_file_path = "../../data2/tom/RNA/DESeq_pval_0.05.tsv"# DESeq_padj.tsv"
...
df['chromosome_id'] = df.apply(lambda row : extract_chrosomosome_reference(row['transcript_reference']), axis = 1)
#chr_dictionary = {}
#position_dictionary = {}
#locus_dictionary = {}
...
df = df.loc[df['transcript_reference'].str.contains("mrna")]
position_dictionary = {}
for item in df['gene_id'].unique():
    #gene_id = get_gene_id(item)
    position_dictionary[item] = get_locus(item)
df['locus'] = df.apply(lambda row : match(row['gene_id'], position_dictionary), axis = 1)
df['chr'] = df.apply(lambda row : match(row['chromosome_id'], chromosome_refseq_dictionary), axis = 1)
...
#df = df.loc[(df['GMM_logit_pvalue'].astype(float) <= 0.005)]
#df = df.loc[(df['relative_average_modification_rate'].astype(float) <= 500) & (df['relative_average_modification_rate'].astype(float) >= -500)]
#df = df.drop_duplicates('transcript_reference', keep='last')
...
for num, (name, group) in enumerate(df_grouped):
    group.plot(kind='scatter', x='ind', y='log2FoldChange', color=colors[num % len(colors)], ax=ax) # y = 'GMM_logit_pvalue', color=colors[num % len(colors)], ax=ax) 
    #y='mod_rate_change'
    #y='number_of_differential_modifications'
...
#ax.set_ylabel('Change in modification rate')
ax.set_ylabel('Log 2 fold change in expression')
ax.set_title("Change in gene expression against genomic position")
#ax.set_title("Modification rate change against position in the genome")
