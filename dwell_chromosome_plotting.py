...
def calculate_dwell_difference(c1_mean_intensity, c2_mean_intensity):
...
output_file_path = "../../../../data2/tom/RNA/dwell_chr.tsv" #"test.tsv"# "../../../../data2/tom/RNA/signal_chr.tsv"
output_graph_path = "../../../../data2/tom/RNA/dwell_chr_manhattan_plot_graph.png" #"test.png" #"../../../../data2/tom/RNA/signal_chr_manhattan_plot_graph.png"
...
df['inter_condition_dwell_difference'] = df.apply(lambda row : calculate_dwell_difference(row['c1_mean_dwell'], row['c2_mean_dwell']), axis = 1)
#df = df.loc[(df['inter_condition_dwell_difference'].astype(float) >= 2)]
...
for num, (name, group) in enumerate(df_grouped):
    group.plot(kind='scatter', x='ind', y='inter_condition_dwell_difference', color=colors[num % len(colors)], ax=ax) # y = 'GMM_logit_pvalue', color=colors[num % len(colors)], ax=ax) 
...
ax.set_ylabel('Difference in mean dwell time')
ax.set_title("Difference in mean dwell time throughout the genome between conditions")
